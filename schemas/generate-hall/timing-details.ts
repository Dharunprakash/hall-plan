...
const timeFormatRegex = /^(0[0-9]|1[0-9]|2[0-3])\.[0-5][0-9]-([0-9]|1[0-9]|2[0-3])\.[0-5][0-9]$/




const validationSchema = z.object({
  time: z.string().min(1, { message: "Time is required" }).refine(value => timeFormatRegex.test(value), {
    message: "Invalid time format. Time format should be in the format 's hh.mm-hh.mm'",
  }),
});
